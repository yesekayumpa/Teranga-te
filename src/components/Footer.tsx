...
// ── Hook responsive (peut être externalisé dans hooks/useBreakpoint.ts) ───
...
export const Footer = () => {
  ...
  // Colonnes du footer : 4 col desktop → 2 col tablet → 1 col mobile
  const gridCols = isDesktop ? 'repeat(4, 1fr)' : isMobile ? '1fr' : 'repeat(2, 1fr)';
  // Délais animations : 0 sur mobile (tout en colonne = pas besoin de cascade)
  ...
  return (
    <footer style={{
      ...
      paddingBottom: 40,
      borderTopLeftRadius: isMobile ? 24 : 48,
      borderTopRightRadius: isMobile ? 24 : 48,
      overflow: 'hidden',
    }}>

      {/* ── Fond texturé + circuits dorés (masqués sur mobile = perfs) ── */}
      {!isMobile && (
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
          ...
          <svg style={{ width: '100%', height: '100%', position: 'absolute', inset: 0, opacity: 0.08 }} viewBox="0 0 1200 600" fill="none">
            <motion.path
              ...
              stroke={PALETTE.primary} strokeWidth="1.5" strokeDasharray="50 70" strokeLinecap="round"
            ...
            <motion.path
              ...
              stroke={PALETTE.primary} strokeWidth="1.5" strokeDasharray="60 80" strokeLinecap="round"
      ...
      {/* ── Effets foudre (masqués sur mobile) ─────────────────────── */}
      {!isMobile && (
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0 }}>
          <motion.div
            style={{ position: 'absolute', top: '25%', left: '25%', width: 1, height: 128, background: PALETTE.primary }}
            animate={{ height: [0, 128, 0], opacity: [0, 0.3, 0] }}
            transition={{ duration: 2, repeat: Infinity, repeatDelay: 5, ease: 'easeInOut' }}
          />
          <motion.div
            style={{ position: 'absolute', top: '33%', right: '33%', width: 1, height: 96, background: PALETTE.primary }}
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: gridCols, gap: isMobile ? 32 : 48, marginBottom: 32 }}>
          ...
          <ScrollReveal style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <motion.div
                ...
                style={{ backgroundColor: PALETTE.text, borderRadius: '50%', padding: 6, display: 'inline-flex', alignItems: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.3)' }}
              >
            ...
            <Link
              to="/contact"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, backgroundColor: PALETTE.primary, color: PALETTE.bg, padding: '12px 24px', borderRadius: 999, fontWeight: 700, fontSize: 14, textDecoration: 'none', alignSelf: 'flex-start', transition: 'background-color 0.2s' }}
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 8 }}>
              {[Linkedin, Twitter, Facebook, Instagram].map((Icon, i) => (
                <motion.a
                  key={i} href="#"
                  style={{ width: 36, height: 36, backgroundColor: PALETTE.card, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}
                  whileHover={{ scale: 1.15, rotate: 5 }}
                  whileTap={{ scale: 0.9 }}
                >
                  <motion.div
                    style={{ position: 'absolute', inset: 0, background: `linear-gradient(135deg, ${PALETTE.primary}44, transparent)`, borderRadius: 12, opacity: 0 }}
          ...
          <ScrollReveal delay={delay(1)}>
            ...
            <ul style={{
              ...
              display: 'grid',
              // 2 cols sur tablet/desktop, 1 col sur mobile
          ...
          <ScrollReveal delay={delay(2)}>
            <FooterHeading label="Expertises" />
          ...
          <ScrollReveal delay={delay(3)}>
            <FooterHeading label="Veille Tech" />
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <motion.input
                ...
                style={{ flex: 1, minWidth: 0, backgroundColor: PALETTE.card, border: `1px solid ${PALETTE.border}`, borderRadius: 12, padding: '8px 16px', fontSize: 11, outline: 'none', color: PALETTE.text, transition: 'border-color 0.2s' }}
                whileFocus={{ boxShadow: `0 0 0 2px ${PALETTE.primary}4d` }}
              />
              <motion.button
                style={{ flexShrink: 0, backgroundColor: PALETTE.primary, border: 'none', borderRadius: 12, padding: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
        ...
        <motion.div
          style={{ height: 1, background: `linear-gradient(90deg, transparent, ${PALETTE.primary}, transparent)`, margin: '32px 0', transformOrigin: 'left', scaleX: 0 }}
          initial={{ scaleX: 0 }}
        ...
        {/* ── Copyright ────────────────────────────────────────────── */}
        <motion.div
          style={{ textAlign: 'center', color: PALETTE.muted, fontSize: isMobile ? 9 : 10, fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase' }}
          initial={{ opacity: 0 }}
        ...
        {/* ── Éclair décoratif (masqué sur mobile) ─────────────────── */}
        {!isMobile && (
          <motion.div
